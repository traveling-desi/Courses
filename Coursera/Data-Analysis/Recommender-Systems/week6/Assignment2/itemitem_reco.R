...
#input_item_list <- c("680","77")
input_item_list <- c("12","8587")
...
return(cos_similarity[,"cosine"])

}

Tot_similarity <- mapply(final_fun,1:length(input_item_list))
rownames(Tot_similarity) <- rownames(item_ratings_norm)

Tot_similarity_sum <- apply(Tot_similarity,1,sum)
Tot_similarity_sum <- Tot_similarity_sum[order(Tot_similarity_sum,decreasing=TRUE)][-(1:length(input_item_list))][1:20]


print_fun <- function(c) {
print(Tot_similarity_sum[c])
print(movies[movies$movie_id==names(Tot_similarity_sum[c]),"movie_name"],max.levels=0)
}

mapply(print_fun,1:5)
