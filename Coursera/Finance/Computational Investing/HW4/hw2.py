...
def findEvents(symbols, startday,endday, marketSymbol, ord_file, verbose=False):
	...
	try:
		for symbol in symbols:
	    		for i in range(1,len(close[symbol])):
	        		if close[symbol][i-1] >= 7 and close[symbol][i] < 7 : # Event
             				np_eventmat[symbol][i] = 1.0  #overwriting by the bit, marking the event
					s = str(timestamps[i]) + "," + (str(symbol) + ",BUY," + "100")
					f.write(s+'\n')
					if i+5 > len(timestamps):
						i = len(timestamps)-6
					s = str(timestamps[i+5]) + "," + (str(symbol) + ",SELL," + "100")
					f.write(s+'\n')
	finally:
        	f.close()
...
eventMatrix = findEvents(symbols,startday,endday,marketSymbol='SPY',ord_file = orders_file,verbose=True)
