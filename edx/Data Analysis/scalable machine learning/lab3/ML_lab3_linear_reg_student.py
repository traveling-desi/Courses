...
def parsePoint(line):
    ...
    Returns:
        LabeledPoint: The line is converted into a `LabeledPoint`, which consists of a label and
            features.
    """
    samplePointList = line.split(',')
    return LabeledPoint(samplePointList[0], samplePointList[1:])
...
# onlyLabels.min()#### **(1d) Shift labels **
...
def calcRMSE(labelsAndPreds):
    ...
    """
    mse = labelsAndPreds.map(lambda (l, p): squaredError(l,p)).mean()
...
def gradientSummand(weights, lp):
    ...
    """
    label = lp.label
    featuresDense = DenseVector(lp.features)
    error = weights.dot(featuresDense) - label
    summand = error*featuresDense
    return summand
...
def getLabeledPrediction(weights, observation):
    ...
    featuresDense = DenseVector(observation.features)
    prediction = weights.dot(featuresDense)
...
def twoWayInteractions(lp):
    ...
    """
    label = lp.label
    featuresVec = lp.features
    for (x,y) in itertools.product(featuresVec, featuresVec) :
        featuresVec = np.hstack( (featuresVec, x*y) )
