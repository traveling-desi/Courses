...
#print dayGroupedHosts.take(1)

dayHostCount = dayGroupedHosts.map(lambda (k,v): (k,set(v)))

#print dayHostCount.take(10)


dailyHosts = dayHostCount.map(lambda (k,v): (k, len(v))).sortByKey().cache()
...
daysWithHosts = dailyHosts.map(lambda (k,v) : k).take(32)
hosts = dailyHosts.map(lambda (k,v) : v).take(32)
...
daysWithAvg = avgDailyReqPerHost.map(lambda (k,v) : k).take(32)
avgs = avgDailyReqPerHost.map(lambda (k,v) : v).take(32)
...
#print badEndpoints.take(10)

badUniqueEndpoints = badEndpoints.reduceByKey(sumTwo).map(lambda (k,v): k)

#print badUniqueEndpoints.takeOrdered(10)
...
badEndpointsSum = badEndpointsCountPairTuple.reduceByKey(sumTwo)
...
errHostsSum = errHostsCountPairTuple.reduceByKey(sumTwo)
...
errDateSum = errDateCountPairTuple.reduceByKey(sumTwo)

errDateSorted = errDateSum.sortByKey(True).cache()

errByDate = errDateSorted.take(32)
...
daysWithErrors404 = errDateSorted.map(lambda (k,v) : k).take(32)
errors404ByDay = errDateSorted.map(lambda (k,v) : v).take(32)
...
hourRecordsSum = hourCountPairTuple.reduceByKey(sumTwo)

hourRecordsSorted = hourRecordsSum.sortByKey(True).cache()
 

errHourList = hourRecordsSorted.take(25)
...
hoursWithErrors404 = hourRecordsSorted.map(lambda (k,v) : k).take(25)
errors404ByHours = hourRecordsSorted.map(lambda (k,v) : v).take(25)
