...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    avgRatings = sum(IDandRatingsTuple[1])/numRatings    
...
movieIDsWithRatingsRDD = (ratingsRDD
                          .map(lambda (k1,k2,k3): (k2, k3)).groupByKey())
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              .join(movieIDsWithAvgRatingsRDD).map(lambda (k1, (k2, k3)): (k3[1], k2, k3[0])))
...
def computeError(predictedRDD, actualRDD):
    ...
    predictedReformattedRDD = predictedRDD.map(lambda (k1, k2, k3): ((k1, k2), k3))

    # Transform actualRDD into the tuples of the form ((UserID, MovieID), Rating)
    actualReformattedRDD = actualRDD.map(lambda (k1, k2, k3): ((k1, k2), k3))
    
    # Compute the squared error for each matching entry (i.e., the same (User ID, Movie ID) in each
    # RDD) in the reformatted RDDs using RDD transformtions - do not use collect()
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD).map(lambda (k1, k2): math.pow(k2[0]-k2[1], 2)))
...
validationForPredictRDD = validationRDD.map(lambda (k1,k2,k3): (k1,k2))
...
myModel = ALS.train(trainingRDD, rank=8, seed=seed, iterations=iterations,
                      lambda_=regularizationParameter)
testForPredictingRDD = testRDD.map(lambda (k1,k2,k3): (k1,k2))
...
trainingAvgRating = trainingRDD.map(lambda (k1,k2,k3): k3).mean()
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda (k1,k2,k3): (k1, k2, trainingAvgRating))
...
moviesRatedByMe = [x[1] for x in myRatedMovies]
myUnratedMoviesRDD = moviesRDD.filter(lambda (k1, k2): k1 not in moviesRatedByMe).map(lambda (k1, k2): ('0', k1))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda (k1, (k2, k3)): (k1, k2))

# Transform predictedRatingsRDD into an RDD with entries that are pairs of the form (Movie ID, Predicted Rating)
predictedRDD = predictedRatingsRDD.map(lambda (k1, k2, k3): (k2, k3))
...
predictedWithCountsRDD  = predictedRDD.join(movieCountsRDD).map(lambda (k1, k2): (k1, (k2[0], k2[1])))
...
ratingsWithNamesRDD = predictedWithCountsRDD.join(moviesRDD).filter(lambda (k1, ((k2, k3), k4)):k3 > 75).map(lambda (k1, ((k2, k3), k4)): (k2, k4, k3))
