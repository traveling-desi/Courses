...
wordCounts = wordPairs.reduceByKey(sumTwo)
...
# TODO: Replace <FILL IN> with appropriate code
wordCountsCollected = (wordsRDD.map(lambda x: (x,1)).reduceByKey(sumTwo)
                            .collect())
...
# TODO: Replace <FILL IN> with appropriate code
uniqueWords = len(wordCounts.keys().collect())
print uniqueWords
...
def wordCount(wordListRDD):
    ...
    """
    return wordListRDD.map(lambda x: (x,1)).reduceByKey(sumTwo)
...
def removePunctuation(text):
    ...
    """
    #text = text.lstrip(' ').rstrip(' ').lower().translate(string.maketrans("",""), string.punctuation)
