...
def simpleTokenize(string):
    ...
    """
    string = string.lower()
    strList = re.findall(split_regex, string)
...
def tokenize(string):
    ...
    return [x for x in strList if x not in stopwords]
...
def countTokens(vendorRDD):
    ...
    """
    return sum(vendorRDD.map(lambda (k,v) : (k,len(v))).values().collect())
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.top(1, key = lambda (k,v): len(v))
...
def tf(tokens):
    ...
    """
    xLen = 1.0/len(tokens)
    dictTokens = Counter(tokens)
    
    return {k: v * xLen for k, v in dictTokens.items()}
...
def idfs(corpus):
    ...
    #print N
    #uniqueTokens = set([item for sublist in corpus.values().collect() for item in sublist])
...
#print idfsSmall.filter(lambda x: x[0] == 'autocad').collect()
...
def tfidf(tokens, idfs):
    ...
    tfs = tf(tokens)
    #print tfs
    tfIdfDict = { x: tfs[x]*idfs[x] for x in tfs.keys()}
    #for x in tfs.keys():
    #    print x, idfs[x]
    return tfIdfDict
...
#for line in crossSmall.take(10):
#    print line
...
#for line in similarities.take(10):
#    print line
...
#for line in trueDupsRDD.take(10):
#    print line

trueDupsCount = float(trueDupsRDD.count())
avgSimDups = sum(trueDupsRDD.map(lambda (k, record): record[0]).collect())/trueDupsCount

#nonDupsRDD = sims.fullOuterJoin(goldStandard).subtractByKey(trueDupsRDD)
nonDupsRDD = sims.fullOuterJoin(goldStandard).filter(lambda (k, record): record[1] is None)
#nonDupsRDD = sims.fullOuterJoin(goldStandard)

#for line in nonDupsRDD.take(10):
#    print line

avgSimNon = sum(nonDupsRDD.map(lambda (k, record): record[0]).collect())/float(nonDupsRDD.count())
...
#for line in fullCorpusRDD.take(10):
#    print line
...
#for line in idfsFullWeights:
#    print line, idfsFullWeights[line]
idfsFullBroadcast = sc.broadcast(idfsFullWeights)

# Pre-compute TF-IDF weights.  Build mappings from record ID weight vector.
amazonWeightsRDD = amazonFullRecToToken.map(lambda (k,v): (k, tfidf(tokenize(' '.join(v)), idfsFullBroadcast.value)))
googleWeightsRDD = googleFullRecToToken.map(lambda (k,v): (k, tfidf(tokenize(' '.join(v)), idfsFullBroadcast.value)))
...
def invert(record):
    ...
    #print record[0]
    #print record[1]
    pairs = [ (tokens, record[0] ) for tokens in record[1].keys() ]
    return (pairs)


#for line in amazonWeightsRDD.take(5):
#    print line

    
record = ('b000jz4hqo', {'rom': 2.4051362683438153, 'clickart': 56.65432098765432, '950': 254.94444444444443, 'image': 3.6948470209339774, 'premier': 9.27070707070707, '000': 6.218157181571815, 'dvd': 1.287598204264871, 'broderbund': 22.169082125603865, 'pack': 2.98180636777128})
...
#for line in googleInvPairsRDD.take(10):
#    print line
...
def fastCosineSimilarity(record):
    ...
    for token in tokens:
        sum_s = sum_s + amazonWeightsBroadcast.value.get(amazonRec)[token] * googleWeightsBroadcast.value.get(googleRec)[token]

    amazonNorm = amazonNormsBroadcast.value.get(amazonRec)
    googleNorm = googleNormsBroadcast.value.get(googleRec)
    
    value = sum_s/(amazonNorm * googleNorm)
